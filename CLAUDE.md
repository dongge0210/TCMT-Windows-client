...
- When your changes create orphans: remove what YOUR change made unused, not pre-existing dead code.
...
C++ Core (TCMT-M / TCMT.exe)
  ...
  ▼
IPCDataBlock (mmap file, schema-broadcast via UDS / Named Pipe)
  │  C++: IPCServer (macOS UDS, TODO: Windows NamedPipeServer)
  │  C#: IPCPipeClient receives Schema → IPCMemoryReader reads fields
  │
  ├── AvaloniaUI (.NET 10.0, cross-platform) — reads via IPCServices
...
- `src/core/IPC/` — IPC pipeline: IPCServer (UDS), SharedMemoryManager, IPCDataBlock
